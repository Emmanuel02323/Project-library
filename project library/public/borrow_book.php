...
redirectIfNotAuthenticated('login.php'); // Include database configuration file
...
$check_stmt->bind_param("i", $book_id); $check_stmt->execute();
// Bind result variable
$check_stmt->bind_result($available_copies); $check_stmt->fetch();
$check_stmt->close();
 // Check if there are available copies
if ($available_copies > 0) {
// Update the available copies
$update_stmt = $conn->prepare("UPDATE books SET available_copies =
available_copies - 1 WHERE id = ?"); $update_stmt->bind_param("i", $book_id); $update_stmt->execute(); $update_stmt->close();
...
try { $borrow_stmt->execute(); // Set success message
 $_SESSION['message'] = "Book borrowed successfully!"; } catch (mysqli_sql_exception $e) {
        // Set error message
